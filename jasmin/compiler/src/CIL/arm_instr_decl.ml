...
let arm_ADD_semi wn wm =
  nzcv_w_of_aluop arm_decl.reg_size
    (add_word arm_decl.reg_size (Obj.magic wn) (Obj.magic wm))
    (Z.add (wunsigned arm_decl.reg_size (Obj.magic wn))
      (wunsigned arm_decl.reg_size (Obj.magic wm)))
    (Z.add (wsigned arm_decl.reg_size (Obj.magic wn))
      (wsigned arm_decl.reg_size (Obj.magic wm)))
...
let arm_ADC_semi wn wm cf =
  let c = Z.b2z cf in
  nzcv_w_of_aluop arm_decl.reg_size
    (add_word arm_decl.reg_size
      (add_word arm_decl.reg_size (Obj.magic wn) (Obj.magic wm))
      (wrepr arm_decl.reg_size c))
    (Z.add
      (Z.add (wunsigned arm_decl.reg_size (Obj.magic wn))
        (wunsigned arm_decl.reg_size (Obj.magic wm))) c)
    (Z.add
      (Z.add (wsigned arm_decl.reg_size (Obj.magic wn))
        (wsigned arm_decl.reg_size (Obj.magic wm))) c)
...
let arm_MUL_semi wn wm =
  let res = mul_word arm_decl.reg_size (Obj.magic wn) (Obj.magic wm) in
...
let arm_MLA_semi wn wm wa =
  Obj.magic add_word arm_decl.reg_size
    (mul_word arm_decl.reg_size (Obj.magic wn) (Obj.magic wm)) wa
...
let arm_MLS_semi wn wm wa =
  Obj.magic sub_word arm_decl.reg_size wa
    (mul_word arm_decl.reg_size (Obj.magic wn) (Obj.magic wm))
...
let arm_SDIV_semi wn wm =
  Obj.magic wdivi arm_decl.reg_size wn wm
...
let arm_SUB_semi wn wm =
  let wmnot = wnot arm_decl.reg_size (Obj.magic wm) in
  nzcv_w_of_aluop arm_decl.reg_size
    (add_word arm_decl.reg_size
      (add_word arm_decl.reg_size (Obj.magic wn) wmnot)
      (word1 arm_decl.reg_size))
    (Z.add
      (Z.add (wunsigned arm_decl.reg_size (Obj.magic wn))
...
let arm_SBC_semi wn wm cf =
  arm_ADC_semi wn (Obj.magic wnot arm_decl.reg_size wm) cf
...
let arm_UDIV_semi wn wm =
  Obj.magic wdiv arm_decl.reg_size wn wm
...
let arm_UMULL_semi wn wm =
  let (hi, lo) = wumul arm_decl.reg_size (Obj.magic wn) (Obj.magic wm) in
  Obj.magic (lo, hi)
...
let arm_UMAAL_semi wa wb wn wm =
  let r =
    Z.add
      (Z.add (wunsigned arm_decl.reg_size (Obj.magic wa))
        (wunsigned arm_decl.reg_size (Obj.magic wb)))
      (Z.mul (wunsigned arm_decl.reg_size (Obj.magic wn))
        (wunsigned arm_decl.reg_size (Obj.magic wm)))
  in
...
let arm_SMULL_semi wn wm =
  let (hi, lo) = wsmul arm_decl.reg_size (Obj.magic wn) (Obj.magic wm) in
  Obj.magic (lo, hi)
...
let arm_SMMUL_semi wn wm =
  Obj.magic wmulhs arm_decl.reg_size wn wm
...
let arm_SMMULR_semi wn wm =
  Obj.magic high_bits arm_decl.reg_size
    (Z.add
      (Z.mul (wsigned arm_decl.reg_size (Obj.magic wn))
        (wsigned arm_decl.reg_size (Obj.magic wm))) (Zpos (Coq_xO (Coq_xO
...
(** val get_hw :
    halfword -> (register, empty, empty, rflag, condt) wreg -> word **)

let get_hw hw x =
  match split_vec arm_decl.reg_size (S (S (S (S (S (S (S (S (S (S (S (S (S (S
          (S (S O)))))))))))))))) (Obj.magic x) with
  | [] -> word0 U16
  | lo :: l ->
    (match l with
     | [] -> word0 U16
...
let arm_smul_hw_semi hwn hwm wn wm =
  ...
  let r = Z.mul (wsigned U16 n) (wsigned U16 m) in Obj.magic wrepr U32 r
...
let arm_smla_hw_semi hwn hwm wn wm acc =
  ...
  let r =
    Z.add (Z.mul (wsigned U16 n) (wsigned U16 m))
      (wsigned arm_decl.reg_size (Obj.magic acc))
  in
  Obj.magic wrepr U32 r
...
let arm_smulw_hw_semi hw wn wm =
  ...
  let res = Z.mul (wsigned arm_decl.reg_size (Obj.magic wn)) (wsigned U16 m)
  in
  let w = wrepr U64 res in
  Obj.magic winit U32 (fun i ->
    wbit_n U64 w
...
(** val arm_bitwise_semi :
    ...
    sem_tuple -> sem_tuple -> sem_tuple **)

let arm_bitwise_semi ws op0 op1 op wn wm =
  let res = op (Obj.magic op0 wn) (Obj.magic op1 wm) in
  Obj.magic ((Some (coq_NF_of_word ws res)), ((Some (coq_ZF_of_word ws res)),
    (None, res)))
...
(** val arm_BFC_semi :
    (register, empty, empty, rflag, condt) wreg -> word -> word -> (register,
       ...
       then if Z.leb nbits
            ...
            then let msbit = Z.sub (Z.add lsbit nbits) (Zpos Coq_xH) in
                 let mk = fun i ->
                   ...
                   then false
                   else wbit_n arm_decl.reg_size (Obj.magic x) i
                 in
                 Ok (Obj.magic winit arm_decl.reg_size mk)
...
(** val arm_BFI_semi :
    (register, empty, empty, rflag, condt) wreg -> (register, empty, empty,
    rflag, condt) wreg -> word -> word -> (register, empty, empty, rflag,
    condt) wreg exec **)
       ...
       then if Z.leb nbits
            ...
            then let msbit = Z.sub (Z.add lsbit nbits) (Zpos Coq_xH) in
                 let mk = fun i ->
                   if (&&) (Nat.leb (Z.to_nat lsbit) i)
                        (Nat.leb i (Z.to_nat msbit))
                   then wbit_n arm_decl.reg_size (Obj.magic y)
                          (subn i (Z.to_nat lsbit))
                   else wbit_n arm_decl.reg_size (Obj.magic x) i
                 in
                 Ok (Obj.magic winit arm_decl.reg_size mk)
            else Error E.no_semantics
...
let arm_MVN_semi wn =
  let res = wnot arm_decl.reg_size (Obj.magic wn) in
  Obj.magic ((Some (coq_NF_of_word arm_decl.reg_size res)), ((Some
...
let arm_shift_semi op op_c wn wsham =
  ...
  else Obj.magic ((Some (coq_NF_of_word arm_decl.reg_size (Obj.magic res))),
         ((Some (coq_ZF_of_word arm_decl.reg_size (Obj.magic res))), ((Some
         (op_c wn sham)), res)))
...
let arm_ASR_C wn shift =
  ...
  then msb arm_decl.reg_size (Obj.magic wn)
  else wbit_n arm_decl.reg_size (Obj.magic wn)
         (Z.to_nat (Z.sub shift (Zpos Coq_xH)))

(** val arm_ASR_semi : sem_tuple -> word -> sem_tuple **)

let arm_ASR_semi wn wsham =
  arm_shift_semi (Obj.magic wsar arm_decl.reg_size) arm_ASR_C wn wsham
...
let arm_LSL_C wn shift =
  ...
  then wbit_n arm_decl.reg_size (Obj.magic wn)
         (subn (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S
           (S (S (S (S (S (S (S (S (S (S (S O))))))))))))))))))))))))))))))))
           (Z.to_nat shift))
  else false

(** val arm_LSL_semi : sem_tuple -> word -> sem_tuple **)

let arm_LSL_semi wn wsham =
  arm_shift_semi (Obj.magic wshl arm_decl.reg_size) arm_LSL_C wn wsham
...
(** val arm_LSR_semi : sem_tuple -> word -> sem_tuple **)

let arm_LSR_semi wn wsham =
  arm_shift_semi (Obj.magic wshr arm_decl.reg_size) arm_LSR_C wn wsham
...
let arm_ROR_C wn shift =
  let res = wror arm_decl.reg_size (Obj.magic wn) shift in
  msb arm_decl.reg_size res

(** val arm_ROR_semi : sem_tuple -> word -> sem_tuple **)

let arm_ROR_semi wn wsham =
  arm_shift_semi (Obj.magic wror arm_decl.reg_size) arm_ROR_C wn wsham
...
let arm_REV_semi w =
  Obj.magic wbswap arm_decl.reg_size w

(** val arm_REV16_semi : sem_tuple -> sem_tuple **)

let arm_REV16_semi w =
  Obj.magic lift1_vec U16 (wbswap U16) U32 w

(** val arm_REVSH_semi : sem_tuple -> sem_tuple **)

let arm_REVSH_semi w =
  Obj.magic sign_extend U32 U16
    (wbswap U16 (zero_extend U16 arm_decl.reg_size (Obj.magic w)))
...
let arm_MOV_semi wn =
  Obj.magic ((Some (coq_NF_of_word arm_decl.reg_size (Obj.magic wn))), ((Some
    (coq_ZF_of_word arm_decl.reg_size (Obj.magic wn))), (None, wn)))
...
(** val arm_MOVT_semi : sem_tuple -> word -> sem_tuple **)
...
let arm_MOVT_semi wn wm =
  ...
  Obj.magic wor arm_decl.reg_size hi
    (wand arm_decl.reg_size (Obj.magic wn) mask)
...
(** val bit_field_extract_semi :
    ...
    empty, empty, rflag, condt) wreg) -> (register, empty, empty, rflag,
    condt) wreg -> word -> word -> (register, empty, empty, rflag, condt)
    wreg exec **)
...
let bit_field_extract_semi shr wn widx wwidth =
  ...
  then Ok
         (Obj.magic shr
           (wshl arm_decl.reg_size (Obj.magic wn)
             (Z.sub
               (Z.sub (Zpos (Coq_xO (Coq_xO (Coq_xO (Coq_xO (Coq_xO
                 Coq_xH)))))) width) idx))
...
(** val extend_bits_semi :
    coq_Z -> (register, empty, empty, rflag, condt) wreg -> word ->
...
let extend_bits_semi len wn wroram =
  ...
  Obj.magic wand arm_decl.reg_size mask
    (wror arm_decl.reg_size (Obj.magic wn) roram)
...
let arm_TST_semi wn wm =
  let res = wand arm_decl.reg_size (Obj.magic wn) (Obj.magic wm) in
  Obj.magic ((Some (coq_NF_of_word arm_decl.reg_size res)), ((Some
...
(** val arm_extend_semi : wsize -> bool -> wsize -> word -> word **)
