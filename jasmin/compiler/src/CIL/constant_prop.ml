...
(** val e2word : wsize -> pexpr -> word exec **)
...
| Coq_cword sz -> Obj.magic e2word sz
...
let to_expr t0 b =
  ...
  | Coq_cword sz -> Ok (wconst sz (Obj.magic b))
...
let sadd_w sz e1 e2 =
  ...
  | Some n ->
    (match is_wconst sz e2 with
     | Some n0 -> wconst sz (add_word sz n n0)
     | None ->
       if eq_op (word_word__canonical__eqtype_Equality sz) (Obj.magic n)
            (Obj.magic word0 sz)
       then e2
...
let ssub_w sz e1 e2 =
  ...
  | Some n1 ->
    (match is_wconst sz e2 with
     | Some n -> wconst sz (sub_word sz n1 n)
...
let smul_w sz e1 e2 =
  ...
  | Some n ->
    (match is_wconst sz e2 with
     | Some n0 -> wconst sz (mul_word sz n n0)
     | None ->
       if eq_op (word_word__canonical__eqtype_Equality sz) (Obj.magic n)
            (Obj.magic word0 sz)
       then wconst sz (word0 sz)
       else if eq_op (word_word__canonical__eqtype_Equality sz) (Obj.magic n)
                 (Obj.magic word1 sz)
            then e2
          ...
          (match is_wconst sz e1 with
           | Some i1 ->
             (match is_wconst sz e2 with
              | Some i2 ->
                Pbool
                  (eq_op (word_word__canonical__eqtype_Equality sz)
                    (Obj.magic i1) (Obj.magic i2))
              | None -> Papp2 ((Oeq ty), e1, e2))
           | None -> Papp2 ((Oeq ty), e1, e2)))
...
let s_opN fcp op es =
  ...
  | Opack (ws, p) ->
    (match app_sopn
             (fst ((map eval_atype (fst (type_of_opN (Opack (ws, p))))),
               (eval_atype (snd (type_of_opN (Opack (ws, p)))))))
             (sem_opN_typed fcp (Opack (ws, p))) es with
     | Ok w -> Papp1 ((Oword_of_int ws), (Pconst (wunsigned ws w)))
     | Error _ -> PappN (op, es))
  | Oarray _ -> PappN (op, es)
  | Ocombine_flags c ->
    (match app_sopn
             (fst ((map eval_atype (fst (type_of_opN (Ocombine_flags c)))),
               (eval_atype (snd (type_of_opN (Ocombine_flags c))))))
             (sem_opN_typed fcp (Ocombine_flags c)) es with
     | Ok b -> Pbool b
     | Error _ -> PappN (op, es))
  ...
  | Cword (sz1, w1) ->
    (match c2 with
     | Cword (sz2, w2) ->
       ...
       then eq_op (word_word__canonical__eqtype_Equality sz2) (Obj.magic w1)
              (Obj.magic w2)
       else false
     | _ -> false)
...
let const_v_eq_axiom _top_assumption_ =
  let _evar_0_ = fun b1 __top_assumption_ ->
    ...
    (match __top_assumption_ with
     ...
     | Cint z -> _evar_0_0 z
     | Cword (sz, word2) -> _evar_0_1 sz word2)
  in
  let _evar_0_0 = fun z1 __top_assumption_ ->
    ...
    (match __top_assumption_ with
     ...
     | Cword (sz, word2) -> _evar_0_2 sz word2)
  in
  let _evar_0_1 = fun sz1 w1 __top_assumption_ ->
    ...
    let _evar_0_3 = fun sz2 w2 ->
      let _evar_0_3 = fun _ ->
        internal_eq_rew_r_dep sz1 sz2 (fun w3 ->
          iffP (eq_op (word_word__canonical__eqtype_Equality sz2) w3 w2)
            (if eq_op (word_word__canonical__eqtype_Equality sz2) w3 w2
             then ReflectT
             else ReflectF)) w1
      in
    ...
    (match __top_assumption_ with
     | Cbool b -> _evar_0_1 b
     | Cint z -> _evar_0_2 z
     | Cword (sz, word2) -> Obj.magic _evar_0_3 sz word2)
  in
  (match _top_assumption_ with
   | Cbool b -> Obj.magic _evar_0_ b
   | Cint z -> Obj.magic _evar_0_0 z
   | Cword (sz, word2) -> Obj.magic _evar_0_1 sz word2)
...
let const_prop_i fcp msfsz asmop gd m i =
    ...
    let ir0 =
      ...
      | Some p ->
        ...
        then Copn ((x :: []), AT_none, (Oslh SLHmove), (e :: []))
               ...
               (wconst msfsz (opp_word msfsz (word1 msfsz))))
  ...
  | Cassert a ->
    let b = const_prop_e fcp without_globals m0 (snd a) in
    (m0, ((MkI (ii, (Cassert ((fst a), b)))) :: []))
  | Cif (b, c1, c2) ->
