...
(** val coq_SF_of_word : wsize -> Word0.word -> bool **)

let coq_SF_of_word =
  msb

(** val coq_PF_of_word : wsize -> Word0.word -> bool **)
...
(** val coq_ZF_of_word : wsize -> Word0.word -> bool **)

let coq_ZF_of_word sz w =
  eq_op (Word0.word_word__canonical__eqtype_Equality sz) (Obj.magic w)
    (Obj.magic Word0.word0 sz)

(** val rflags_of_bwop : wsize -> Word0.word -> sem_tuple **)
...
(** val rflags_of_aluop :
    wsize -> Word0.word -> coq_Z -> coq_Z -> sem_tuple **)
...
(** val rflags_of_andn : wsize -> Word0.word -> sem_tuple **)
...
(** val rflags_of_aluop_nocf : wsize -> Word0.word -> coq_Z -> sem_tuple **)
...
(** val flags_w : __ list -> ltuple -> wsize -> Word0.word -> ltuple **)

let flags_w l1 bs sz w =
  merge_tuple l1 (map (Obj.magic __) (map eval_ltype (w_ty sz))) bs
    (Obj.magic w)
...
(** val x86_MOV : wsize -> Word0.word -> Word0.word **)
...
(** val x86_MOVX : wsize -> Word0.word -> Word0.word **)
...
let x86_POR v1 v2 =
  Obj.magic Word0.wor x86_decl.reg_size v1 v2
...
let coq_Ox86_PADD_instr =
  ...
  (sem_prod_ok (map eval_ltype (w2_ty sz sz))
    (Obj.magic lift2_vec (wsize_of_velem ve)
      (Word0.add_word (wsize_of_velem ve)) sz)); id_args_kinds = check_padd;
...
(** val x86_MOVSX : wsize -> wsize -> Word0.word -> sem_tuple **)

let x86_MOVSX szi szo x =
  Obj.magic sign_extend szo szi x
...
(** val x86_MOVZX : wsize -> wsize -> Word0.word -> sem_tuple **)

let x86_MOVZX szi szo x =
  Obj.magic zero_extend szo szi x
...
(** val x86_XCHG : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_CMOVcc :
    wsize -> bool -> Word0.word -> Word0.word -> sem_tuple **)

let x86_CMOVcc _ b w2 w3 =
  if b then Obj.magic w2 else Obj.magic w3
...
(** val x86_ADD : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_ADD sz v1 v2 =
  rflags_of_aluop_w sz (Word0.add_word sz v1 v2)
...
(** val x86_SUB : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_SUB sz v1 v2 =
  rflags_of_aluop_w sz (Word0.sub_word sz v1 v2)
...
(** val x86_MUL : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_MUL sz v1 v2 =
  let lo = Word0.mul_word sz v1 v2 in
...
(** val x86_IMUL_overflow : wsize -> Word0.word -> Word0.word -> bool **)

let x86_IMUL_overflow sz hi lo =
  let ov = wdwords sz hi lo in
  (||) (Z.ltb ov (wmin_signed sz)) (Z.gtb ov (wmax_signed sz))

(** val x86_IMUL : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_IMULt : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_DIV :
    wsize -> Word0.word -> Word0.word -> Word0.word -> sem_tuple exec **)
...
(** val x86_CQO : wsize -> Word0.word -> Word0.word **)

let x86_CQO sz w =
  if msb sz w then Word0.opp_word sz (Word0.word1 sz) else Word0.word0 sz
...
(** val add_carry : wsize -> coq_Z -> coq_Z -> coq_Z -> Word0.word **)

let add_carry sz x y c0 =
  wrepr sz (Z.add (Z.add x y) c0)

(** val x86_ADC : wsize -> Word0.word -> Word0.word -> bool -> sem_tuple **)
...
(** val sub_borrow : wsize -> coq_Z -> coq_Z -> coq_Z -> Word0.word **)

let sub_borrow sz x y c0 =
  wrepr sz (Z.sub (Z.sub x y) c0)

(** val x86_SBB : wsize -> Word0.word -> Word0.word -> bool -> sem_tuple **)
...
(** val x86_ADCX : wsize -> Word0.word -> Word0.word -> bool -> sem_tuple **)
...
(** val x86_MULX_lo_hi : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_NEG : wsize -> Word0.word -> sem_tuple **)

let x86_NEG sz w =
  let vs = Z.opp (wsigned sz w) in
  let v = Word0.opp_word sz w in
  flags_w (map (Obj.magic __) (map eval_ltype b5_ty))
    (Obj.magic ((Some
      (negb
        (eq_op coq_BinNums_Z__canonical__eqtype_Equality
      ...
      (negb
        (eq_op (Word0.word_word__canonical__eqtype_Equality sz) (Obj.magic w)
          (Obj.magic Word0.word0 sz)))), ((Some (coq_SF_of_word sz v)),
      ((Some (coq_PF_of_word sz v)), (Some (coq_ZF_of_word sz v))))))) sz v
...
(** val x86_INC : wsize -> Word0.word -> sem_tuple **)

let x86_INC sz w =
  rflags_of_aluop_nocf_w sz (Word0.add_word sz w (Word0.word1 sz))
...
(** val x86_DEC : wsize -> Word0.word -> sem_tuple **)

let x86_DEC sz w =
  rflags_of_aluop_nocf_w sz (Word0.sub_word sz w (Word0.word1 sz))
...
(** val x86_LZCNT : wsize -> Word0.word -> sem_tuple **)
...
(** val x86_TZCNT : wsize -> Word0.word -> sem_tuple **)
...
(** val x86_BSR : wsize -> Word0.word -> sem_tuple exec **)
...
let x86_BSR sz w =
  ...
  then Ok
         (flags_w (map (Obj.magic __) (map eval_ltype b5_ty))
           ...
           (Word0.sub_word sz
             (wrepr sz (Z.sub (wsize_bits sz) (Zpos Coq_xH)))
             (leading_zero sz w)))
...
(** val x86_BT : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_LEA : wsize -> Word0.word -> sem_tuple **)
...
(** val x86_TEST : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_CMP : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_CMP sz x y =
  rflags_of_aluop sz (Word0.sub_word sz x y)
...
(** val x86_AND : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_OR : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_XOR : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_ANDN : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_NOT : wsize -> Word0.word -> sem_tuple **)
...
(** val x86_shift_mask : wsize -> Word0.word **)
...
(** val x86_ROR : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_ROR sz v i0 =
  let i1 = Word0.wand U8 i0 (x86_shift_mask sz) in
  if eq_op (Word0.word_word__canonical__eqtype_Equality U8) (Obj.magic i1)
       (Obj.magic Word0.word0 U8)
  then Obj.magic (None, (None, v))
  else let r0 = wror sz v (wunsigned U8 i1) in
       let cF = msb sz r0 in
       let oF =
         if eq_op (Word0.word_word__canonical__eqtype_Equality U8)
              (Obj.magic i1) (Obj.magic Word0.word1 U8)
         then Some
                (negb
...
(** val x86_ROL : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_rotate_with_carry :
    wsize -> (word -> nat -> word) -> (Word0.word -> bool -> bool) ->
    Word0.word -> Word0.word -> bool -> sem_tuple **)
    ...
    t2w (S (S (wsize_size_minus_1 sz)))
      (tuple (S (S (wsize_size_minus_1 sz)))
        (cons_tuple (S (wsize_size_minus_1 sz)) cf
          (w2t (S (wsize_size_minus_1 sz)) v)) (fun _ ->
        cf :: (w2t (S (wsize_size_minus_1 sz)) v)))
  in
...
(** val x86_RCR : wsize -> Word0.word -> Word0.word -> bool -> sem_tuple **)
...
(** val x86_RCL : wsize -> Word0.word -> Word0.word -> bool -> sem_tuple **)
...
(** val rflags_OF :
    wsize -> wsize -> Word0.word -> Word0.word -> bool -> bool -> sem_tuple **)

let rflags_OF s sz i0 r0 rc oF =
  let oF0 =
    if eq_op (Word0.word_word__canonical__eqtype_Equality s) (Obj.magic i0)
         (Obj.magic Word0.word1 s)
    then Some oF
    else None
  in
  let cF = Some rc in
...
(** val x86_SHL : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_SHL sz v i0 =
  let i1 = Word0.wand U8 i0 (x86_shift_mask sz) in
  if eq_op (Word0.word_word__canonical__eqtype_Equality U8) (Obj.magic i1)
       (Obj.magic Word0.word0 U8)
  then rflags_None_w sz (Obj.magic v)
...
(** val x86_SHR : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_SAR : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
let x86_SHLD sz v1 v2 i0 =
  ...
  if eq_op (Word0.word_word__canonical__eqtype_Equality U8) (Obj.magic i1)
       (Obj.magic Word0.word0 U8)
  then Ok (rflags_None_w sz (Obj.magic v1))
...
(** val x86_RORX : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_bmi_shift :
    wsize -> (Word0.word -> coq_Z -> Word0.word) -> Word0.word -> Word0.word
    -> Word0.word **)
...
(** val x86_SARX : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_SHRX : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_SHLX : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_BSWAP : wsize -> Word0.word -> sem_tuple **)

let x86_BSWAP sz v =
  Obj.magic wbswap sz v
...
(** val x86_POPCNT : wsize -> Word0.word -> sem_tuple **)
...
(** val x86_BTX :
    (wsize -> Word0.word -> Word0.word -> sem_ot) -> wsize -> Word0.word ->
    Word0.word -> sem_tuple **)
...
(** val x86_PEXT : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_PEXT sz v1 v2 =
  Obj.magic pextr sz v1 v2
...
(** val x86_PDEP : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_PDEP sz v1 v2 =
  Obj.magic pdep sz v1 v2
...
(** val x86_MOVD : wsize -> Word0.word -> sem_tuple **)

let x86_MOVD sz v =
  Obj.magic zero_extend U128 sz v
...
(** val x86_VMOVDQ : wsize -> Word0.word -> sem_tuple **)
...
(** val x86_VPMOVSX :
    velem -> wsize -> velem -> wsize -> Word0.word -> Word0.word **)
...
(** val x86_VPAND : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_VPANDN : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_VPOR : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_VPXOR : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_VPADD :
    velem -> wsize -> Word0.word -> Word0.word -> Word0.word **)

let x86_VPADD ve sz =
  lift2_vec (wsize_of_velem ve) (Word0.add_word (wsize_of_velem ve)) sz
...
let x86_VPSUB ve sz =
  lift2_vec (wsize_of_velem ve) (fun x y ->
    Word0.sub_word (wsize_of_velem ve) x y) sz
...
let x86_VPMULL ve sz v1 v2 =
  lift2_vec (wsize_of_velem ve) (Word0.mul_word (wsize_of_velem ve)) sz v1 v2
...
(** val x86_VPMUL : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_VPMULU : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_VPMULH : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_VPMULHU : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_VPMULHRS : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
(** val x86_nelem_mask : wsize -> wsize -> Word0.word **)
...
(** val x86_VPEXTR : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VPEXTR ve v i0 =
  let i1 = Word0.wand U8 i0 (x86_nelem_mask ve U128) in
  nth (Obj.magic Word0.word0 ve)
...
(** val x86_VPINSR :
    velem -> Word0.word -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VPINSR ve v1 v2 i0 =
  let i1 = Word0.wand U8 i0 (x86_nelem_mask (wsize_of_velem ve) U128) in
  Obj.magic wpinsr (wsize_of_velem ve) v1 v2 i1
...
(** val x86_u128_shift :
    wsize -> wsize -> (Word0.word -> coq_Z -> Word0.word) -> Word0.word ->
    Word0.word -> sem_tuple **)

let x86_u128_shift sz' sz op v c0 =
  Obj.magic lift1_vec sz' (fun v0 ->
    op v0 (wunsigned U64 (zero_extend U64 U128 c0))) sz v
...
(** val x86_VPSLL :
    velem -> wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
let x86_u128_shift_variable ve sz op v1 v2 =
  Obj.magic lift2_vec ve (fun v3 v4 ->
    op v3 (Z.min (wunsigned ve v4) (wsize_bits ve))) sz v1 v2

(** val x86_VPSLLV :
    wsize -> wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_vpsxldq :
    wsize -> (Word0.word -> Word0.word -> sem_tuple) -> Word0.word ->
    Word0.word -> sem_tuple **)

let x86_vpsxldq _ op =
  op

(** val x86_VPSLLDQ : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VPSLLDQ sz =
  x86_vpsxldq sz (Obj.magic wpslldq sz)
...
(** val x86_VPSRLDQ : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VPSRLDQ sz =
  x86_vpsxldq sz (Obj.magic wpsrldq sz)
...
(** val x86_VPSHUFB : wsize -> Word0.word -> Word0.word -> Word0.word **)
...
let x86_vpshuf _ op v1 v2 =
  Obj.magic op v1 (wunsigned U8 v2)

(** val x86_VPSHUFHW : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_VPSHUFLW : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val x86_VPSHUFD : wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
(** val wpblendw : Word0.word -> Word0.word -> Word0.word -> Word0.word **)
...
let wpblendw m0 w1 w2 =
  ...
  let r0 =
    map3 (fun b0 v3 v4 ->
      if eq_op (word_word__canonical__eqtype_Equality (S O)) (Obj.magic b0)
           (Obj.magic word1 O)
      then v4
      else v3) b v1 v2
  in
  make_vec U16 U128 r0

(** val x86_VPBLEND :
    Equality.sort -> wsize -> Word0.word -> Word0.word -> Word0.word ->
    sem_tuple **)

let x86_VPBLEND ve sz v1 v2 m0 =
  if eq_op wsize_wsize__canonical__eqtype_Equality ve (Obj.magic U32)
  then Obj.magic wpblendd sz v1 v2 m0
  else Obj.magic lift2_vec U128 (wpblendw m0) sz v1 v2
...
(** val x86_BLENDV :
    velem -> wsize -> Word0.word -> Word0.word -> Word0.word -> sem_tuple **)

let x86_BLENDV ve sz x y m0 =
  Obj.magic blendv ve sz x y m0
...
(** val coq_SaturatedSignedToUnsigned :
    wsize -> wsize -> Word0.word -> Word0.word **)
...
(** val vpack2 :
    wsize -> wsize -> wsize -> (Word0.word -> Word0.word) -> Word0.word ->
    Word0.word -> Word0.word **)

let vpack2 sz1 sz2 sz op w1 w2 =
  make_vec sz2 sz
    (cat (map op (split_vec sz (nat_of_wsize sz1) w1))
      (map op (split_vec sz (nat_of_wsize sz1) w2)))

(** val x86_VPACKUS :
    Equality.sort -> wsize -> Word0.word -> Word0.word -> sem_tuple **)
...
let x86_VPACKUS ve sz v1 v2 =
  ...
  if eq_op wsize_wsize__canonical__eqtype_Equality (Obj.magic sz)
       (Obj.magic U128)
  then Obj.magic doit sz v1 v2
  else Obj.magic lift2_vec U128 (doit U128) sz v1 v2
...
(** val wshufps_128 : Word0.word -> Word0.word -> Word0.word -> Word0.word **)

let wshufps_128 o s1 s2 =
  make_vec U32 U128
    ((wpshufd1 s1 o O) :: ((wpshufd1 s1 o (S O)) :: ((wpshufd1 s2 o (S (S O))) :: (
    (wpshufd1 s2 o (S (S (S O)))) :: []))))

(** val x86_VSHUFPS :
    wsize -> Word0.word -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VSHUFPS sz s1 s2 o =
  Obj.magic lift2_vec U128 (wshufps_128 o) sz s1 s2
...
(** val x86_VPBROADCAST : wsize -> wsize -> Word0.word -> sem_tuple **)

let x86_VPBROADCAST ve sz v =
  Obj.magic wpbroadcast ve sz v
...
(** val x86_VMOVSHDUP : wsize -> Word0.word -> sem_tuple **)

let x86_VMOVSHDUP sz v =
  Obj.magic wdup_hi (wsize_of_velem VE32) sz v
...
(** val x86_VMOVSLDUP : wsize -> Word0.word -> sem_tuple **)

let x86_VMOVSLDUP sz v =
  Obj.magic wdup_lo (wsize_of_velem VE32) sz v
...
(** val x86_VPALIGNR128 :
    Word0.word -> Word0.word -> Word0.word -> Word0.word **)
...
let x86_VPALIGNR128 m0 v1 v2 =
  ...
  in
  nth (Word0.word0 U128) (split_vec U256 (nat_of_wsize U128) v') O

(** val x86_VPALIGNR :
    wsize -> Word0.word -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VPALIGNR sz v1 v2 m0 =
  Obj.magic lift2_vec U128 (x86_VPALIGNR128 m0) sz v1 v2
...
(** val x86_VINSERTI128 :
    Word0.word -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VINSERTI128 v1 v2 m0 =
  Obj.magic winserti128 v1 v2 m0
...
(** val x86_VPERMD : Word0.word -> Word0.word -> sem_tuple **)

let x86_VPERMD v1 v2 =
  Obj.magic wpermd U256 v1 v2
...
(** val x86_VPERMQ : Word0.word -> Word0.word -> sem_tuple **)

let x86_VPERMQ v m0 =
  Obj.magic wpermq v m0
...
let x86_VPCMPEQ ve sz v1 v2 =
  Obj.magic wpcmpeq (wsize_of_velem ve) sz v1 v2
...
let x86_VPCMPGT ve sz v1 v2 =
  Obj.magic wpcmpgt (wsize_of_velem ve) sz v1 v2
...
let x86_VPSIGN ve sz v1 v2 =
  Obj.magic lift2_vec (wsize_of_velem ve) (fun x m0 ->
    match wsigned (wsize_of_velem ve) m0 with
    | Z0 -> Word0.word0 (wsize_of_velem ve)
    | Zpos _ -> x
    | Zneg _ -> Word0.opp_word (wsize_of_velem ve) x) sz v1 v2
...
(** val x86_VPMADDUBSW : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VPMADDUBSW sz v v1 =
  Obj.magic wpmaddubsw sz v v1
...
(** val x86_VPMADDWD : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VPMADDWD sz v v1 =
  Obj.magic wpmaddwd sz v v1
...
(** val x86_VMOVLPD : Word0.word -> sem_tuple **)

let x86_VMOVLPD v =
  Obj.magic zero_extend U64 U128 v
...
(** val x86_VMOVHPD : Word0.word -> sem_tuple **)

let x86_VMOVHPD v =
  Obj.magic zero_extend U64 U128
...
let x86_VPMINS ve sz x y =
  Obj.magic wmin Signed (wsize_of_velem ve) sz x y
...
let x86_VPMINU ve sz x y =
  Obj.magic wmin Unsigned (wsize_of_velem ve) sz x y
...
let x86_VPMAXS ve sz x y =
  Obj.magic wmax Signed (wsize_of_velem ve) sz x y
...
let x86_VPMAXU ve sz x y =
  Obj.magic wmax Unsigned (wsize_of_velem ve) sz x y
...
(** val x86_VPABS : velem -> wsize -> Word0.word -> sem_tuple **)

let x86_VPABS ve sz x =
  Obj.magic wabs ve sz x
...
(** val x86_VPTEST : wsize -> Word0.word -> Word0.word -> sem_tuple **)

let x86_VPTEST sz x y =
  Obj.magic ((Some false), ((Some
    (eq_op (Word0.word_word__canonical__eqtype_Equality sz)
      (Obj.magic wandn sz x y) (Obj.magic Word0.word0 sz))), ((Some false),
...
(** val x86_AESDEC : Word0.word -> Word0.word -> sem_tuple **)

let x86_AESDEC v1 v2 =
  Obj.magic wAESDEC v1 v2

(** val x86_AESDECLAST : Word0.word -> Word0.word -> sem_tuple **)

let x86_AESDECLAST v1 v2 =
  Obj.magic wAESDECLAST v1 v2

(** val x86_AESENC : Word0.word -> Word0.word -> sem_tuple **)

let x86_AESENC v1 v2 =
  Obj.magic wAESENC v1 v2

(** val x86_AESENCLAST : Word0.word -> Word0.word -> sem_tuple **)

let x86_AESENCLAST v1 v2 =
  Obj.magic wAESENCLAST v1 v2

(** val x86_AESIMC : Word0.word -> sem_tuple **)

let x86_AESIMC v1 =
  Obj.magic coq_InvMixColumns v1

(** val x86_AESKEYGENASSIST : Word0.word -> Word0.word -> sem_tuple **)

let x86_AESKEYGENASSIST v1 v2 =
  Obj.magic wAESKEYGENASSIST v1 v2
...
(** val mk_instr_aes3 :
    string -> string -> (wsize -> x86_op) -> (Word0.word -> Word0.word ->
    Word0.word) -> (wsize -> (register, register_ext, xmm_register, rflag,
    condt) instr_desc_t) * (string * x86_op prim_constructor) **)
...
(** val wclmulq : Word0.word -> Word0.word -> Word0.word **)

let wclmulq x1 x2 =
  let x = zero_extend U128 U64 x1 in
  foldr (fun k r0 ->
    Word0.wxor U128
      (if wbit_n U64 x2 k then wshl U128 x (Z.of_nat k) else Word0.word0 U128)
      r0) (Word0.word0 U128)
...
(** val wVPCLMULDQD :
    wsize -> Word0.word -> Word0.word -> Word0.word -> Word0.word **)

let wVPCLMULDQD sz w1 w2 k =
  let get1 = fun w ->
    nth (Word0.word0 U64) (split_vec U128 (nat_of_wsize U64) w)
  ...
  in
  let f = fun w3 w4 -> wclmulq (get1 w3) (get2 w4) in
  make_vec U128 sz
    (map2 f (split_vec sz (nat_of_wsize U128) w1)
...
let x86_VPCLMULQDQ sz v1 v2 k =
  Obj.magic wVPCLMULDQD sz v1 v2 k
