...
module E =
 struct
  ...
  (** val tunneling_error : string -> pp_error_loc **)
...
let setfb _ fd fb =
  { lfd_info = fd.lfd_info; lfd_align = fd.lfd_align; lfd_tyin = fd.lfd_tyin;
    ...
    fd.lfd_callee_saved; lfd_stk_max = fd.lfd_stk_max; lfd_frame_size =
    fd.lfd_frame_size; lfd_align_args = fd.lfd_align_args }

(** val setfuncs :
    'a1 asmOp -> 'a1 lprog -> (funname * 'a1 lfundef) list -> 'a1 lprog **)

let setfuncs _ p lf =
  { lp_rip = p.lp_rip; lp_rsp = p.lp_rsp; lp_globs = p.lp_globs;
    lp_glob_names = p.lp_glob_names; lp_funcs = lf }
...
let well_formed_body asmop fn fb =
  ...
  (&&)
    (uniq coq_BinNums_positive__canonical__eqtype_Equality (Obj.magic lbls))
    (all (fun pat ->
      let (fn', l) = pat in
      (||) (negb (eq_op funname_eqType (Obj.magic fn) (Obj.magic fn')))
        (in_mem (Obj.magic l)
          (mem
            (seq_predType coq_BinNums_positive__canonical__eqtype_Equality)
            (Obj.magic lbls)))) (goto_targets asmop fb))
...
let tunnel_program asmop p =
  ...
  else Error (E.tunneling_error "not well-formed")
