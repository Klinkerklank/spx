  ...
  let rm_glob_error_gen ii x extra =
    { pel_msg =
      (pp_box ((PPEstring "Cannot remove global variable") :: ((PPEvar
        x) :: extra))); pel_fn = None; pel_fi = None; pel_ii = (Some ii);
...
let add_glob ii x gd gv0 =
  ...
  then Ok gd
            ...
            eq_op Var.coq_MvMake_var__canonical__eqtype_Equality
              (fst (Obj.magic g')) (Obj.magic x)) gd
  ...
  (match i0 with
   ...
   | Cif (_, c1, c2) ->
     (match foldM (extend_glob_i asmop fcp) gd c1 with
      ...
      | Error s -> Error s)
   | Cfor (_, _, c) -> foldM (extend_glob_i asmop fcp) gd c
   | Cwhile (_, c1, _, _, c2) ->
     (match foldM (extend_glob_i asmop fcp) gd c1 with
      | Ok x -> foldM (extend_glob_i asmop fcp) x c2
      | Error s -> Error s)
...
(** val remove_glob_i :
    'a1 asmOp -> coq_FlagCombinationParams -> glob_decl list -> Var.var
    Mvar.t -> 'a1 instr -> (Var.var Mvar.t * 'a1 instr list) cexec **)

let rec remove_glob_i asmop fcp gd env = function
| MkI (ii, i0) ->
  (match i0 with
   | Cassgn (lv, tag, ty, e) ->
     (match remove_glob_e ii env e with
   ...
   | Csyscall (lvs, o, es) ->
     (match mapM (remove_glob_lv ii env) lvs with
      ...
      | Error s -> Error s)
   | Cassert a ->
     (match sndM (remove_glob_e ii env) a with
      | Ok x -> Ok (env, ((MkI (ii, (Cassert x))) :: []))
      | Error s -> Error s)
   | Cif (e, c1, c2) ->
     (match remove_glob_e ii env e with
      | Ok x ->
        (match remove_glob asmop (remove_glob_i asmop fcp gd) env c1 with
         | Ok x0 ->
           let env1 = fst x0 in
           let c3 = snd x0 in
           (match remove_glob asmop (remove_glob_i asmop fcp gd) env c2 with
            | Ok x1 ->
              let env2 = fst x1 in
              let c4 = snd x1 in
             ...
             (match remove_glob_e ii env e2 with
              | Ok x0 ->
                let check_c = fun env0 ->
                  remove_glob asmop (remove_glob_i asmop fcp gd) env0 c
                in
   ...
   | Cwhile (a, c1, e, info, c2) ->
     let check_c = fun env0 ->
       match remove_glob asmop (remove_glob_i asmop fcp gd) env0 c1 with
       | Ok x ->
         let env1 = fst x in
         (match remove_glob_e ii env1 e with
          | Ok x0 ->
            (match remove_glob asmop (remove_glob_i asmop fcp gd) env1 c2 with
...
let remove_glob_fundef asmop fcp gd f =
  ...
  (match mapM check_var f.f_params with
   | Ok _ ->
     (match mapM check_var f.f_res with
      | Ok _ ->
        (match remove_glob asmop (remove_glob_i asmop fcp gd) Mvar.empty
                 f.f_body with
         | Ok x ->
...
(** val remove_glob_prog :
    'a1 asmOp -> coq_FlagCombinationParams -> 'a1 uprog -> (pp_error_loc,
    ('a1, extra_fun_t, extra_prog_t) _prog) result **)

let remove_glob_prog asmop fcp p =
  ...
  | Ok x ->
    ...
    then (match map_cfprog_gen (fun x0 -> x0.f_info)
                  (remove_glob_fundef asmop fcp x) p.p_funcs with
