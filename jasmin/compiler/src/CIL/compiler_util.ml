...
let pp_Sv s =
  pp_nobox ((PPEstring
    "{") :: ((pp_list (pp_break_s ",") (Obj.magic (fun x -> PPEvar x))
               (SvExtra.Sv.elements s)) :: ((PPEstring "}") :: [])))
...
let with_pel_msg e msg =
  { pel_msg = msg; pel_fn = e.pel_fn; pel_fi = e.pel_fi; pel_ii = e.pel_ii;
    pel_vi = e.pel_vi; pel_pass = e.pel_pass; pel_internal = e.pel_internal }
...
(** val pp_internal_error_s_at :
    string -> instr_info -> string -> pp_error_loc **)
...
let gen_loop_iterator pass_name ii =
  { pel_msg = (PPEstring "loop iterator too small"); pel_fn = None; pel_fi =
    None; pel_ii = ii; pel_vi = None; pel_pass = (Some pass_name);
    pel_internal = true }
