...
(** val x86_MULX : wsize -> word -> word -> sem_tuple **)
...
(** val x86_MULX_hi : wsize -> word -> word -> sem_tuple **)

let x86_MULX_hi sz v1 v2 =
  Obj.magic wmulhu sz v1 v2
...
let coq_Ox86SLHinit_instr =
  { str = (pp_s coq_Ox86SLHinit_str); tin = []; i_in = []; tout = ((Coq_aword
    ...
    conflicts = []; semi =
    (sem_prod_ok (map eval_atype [])
      (Obj.magic se_init_sem (arch_msfsz x86_decl))); i_valid = true;
    i_safe = [] }

(** val x86_se_update_sem : bool -> word -> word * word **)
...
(** val se_protect_small_sem : wsize -> word -> word -> sem_tuple **)

let se_protect_small_sem =
  x86_OR

(** val se_protect_mmx_sem : wsize -> word -> word -> word **)

let se_protect_mmx_sem =
  wor

(** val se_protect_large_sem : wsize -> word -> word -> word * word **)
