...
module E =
 struct
  ...
  (** val dead_code_error : string -> pp_error_loc **)
     ...
     | Pvar x2 ->
       (&&) (is_lvar x2)
         (eq_op Var.coq_MvMake_var__canonical__eqtype_Equality
           (Obj.magic x1.v_var) (Obj.magic x2.gv.v_var))
     | _ -> false)
  | _ -> false
...
let rec check_keep_only xs tokeep s =
  ...
  | [] ->
    (match xs with
     | [] -> Ok (s, [])
     | _ :: _ -> Error (E.dead_code_error "check_keep_only invalid size"))
  | b :: tokeep0 ->
    (match xs with
     | [] -> Error (E.dead_code_error "check_keep_only invalid size")
     | x :: xs0 ->
       (match check_keep_only xs0 tokeep0 s with
        | Ok x0 ->
          ...
          then Ok ((read_rv_rec (SvExtra.Sv.diff s0 (vrv x)) x), (x :: xs1))
               ...
               then Ok (s0, xs1)
               else Error (E.dead_code_error "check_keep_only")
        | Error s0 -> Error s0))
...
let rec dead_code_i asmop is_move_op do_nop onfun i s =
  ...
  (match ir with
   | Cassgn (x, tag, _, e) ->
     ...
     if negb
          (eq_op expr_assgn_tag__canonical__eqtype_Equality (Obj.magic tag)
            (Obj.magic AT_keep))
     then if (||) ((&&) (SvExtra.disjoint s w) (negb (lv_write_mem x)))
               ((&&)
                 ((||) do_nop
                   (eq_op expr_assgn_tag__canonical__eqtype_Equality
                     (Obj.magic tag) (Obj.magic AT_rename))) (check_nop x e))
          then Ok (s, [])
   ...
   | Csyscall (xs, _, es) ->
     Ok ((read_es_rec (read_rvs_rec (SvExtra.Sv.diff s (vrvs xs)) xs) es),
       (i :: []))
   | Cassert a -> Ok ((read_e_rec s (snd a)), (i :: []))
   | Cif (b, c1, c2) ->
   ...
   | Cwhile (a, c, e, info, c') ->
     ...
     (match wloop asmop dobody ii Loop.nb s with
      | Ok x ->
        let (s0, y) = x in
        let (c0, c'0) = y in
        Ok (s0, ((MkI (ii, (Cwhile (a, c0, e, info, c'0)))) :: []))
      | Error s0 -> Error s0)
   | Ccall (xs, fn, es) ->
     (match match onfun fn with
      ...
      | Ok x ->
        let (si, xs0) = x in
        Ok ((read_es_rec si es), ((MkI (ii, (Ccall (xs0, fn, es)))) :: []))
      | Error s0 -> Error s0))
