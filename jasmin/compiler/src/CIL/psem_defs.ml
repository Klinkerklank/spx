...
| Pload (al, sz, e0) ->
  (match match sem_pexpr wsw ep spp wdb gd s e0 with
         ...
         | Error s0 -> Error s0 with
   | Ok x ->
     (match CoreMem.read (word_word__canonical__eqtype_Equality ep._pd)
              (coq_PointerW ep._pd) (Memory.coq_CM ep._pd) s.emem al
              (Obj.magic x) sz with
      | Ok x0 -> Ok (to_val (Coq_cword sz) (Obj.magic x0))
      | Error s0 -> Error s0)
   | Error s0 -> Error s0)
| Papp1 (o, e1) ->
    ...
    (match match sem_pexpr wsw ep spp wdb gd s e with
     ...
     | Ok x ->
       (match to_word sz v with
        | Ok x0 ->
          (match CoreMem.write (word_word__canonical__eqtype_Equality ep._pd)
                   (coq_PointerW ep._pd) (Memory.coq_CM ep._pd) s.emem al
                   (Obj.magic x) sz x0 with
