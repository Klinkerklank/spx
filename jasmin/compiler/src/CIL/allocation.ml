...
let rec check_e wsw e1 e2 m =
  ...
  | Papp1 (o1, e3) ->
    (match e2 with
     | Papp1 (o2, e4) ->
       if eq_op operators_sop1__canonical__eqtype_Equality (Obj.magic o1)
            (Obj.magic o2)
       then check_e wsw e3 e4 m
       else Error error_e
     | _ -> Error error_e)
  | Papp2 (o1, e11, e12) ->
    (match e2 with
     | Papp2 (o2, e21, e22) ->
       (match if eq_op operators_sop2__canonical__eqtype_Equality
  ...
  | PappN (o1, es1) ->
    (match e2 with
     | PappN (o2, es2) ->
       if eq_op operators_opN__canonical__eqtype_Equality (Obj.magic o1)
            (Obj.magic o2)
      ...
      (match i2 with
       | Csyscall (xs2, o2, es2) ->
         (match if eq_op syscall_syscall_t__canonical__eqtype_Equality
          ...
          | Error s -> Error s)
       | _ -> Error (alloc_error "instructions not equals"))
    | Cassert _ -> Error (alloc_error "instructions not equals")
    | Cif (e1, c11, c12) ->
      (match i2 with
       | Cif (e2, c21, c22) ->
