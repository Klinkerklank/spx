 ...
 struct
  ...
  let get8 s m i =
    ...
    then if is_init s m i
         then Ok (Ssrfun.Option.default (word0 U8) (Mz.get m (Obj.magic i)))
         else let s0 = ErrAddrUndef in Error s0
    else let s0 = ErrOob in Error s0

  (** val set8 :
      positive -> array -> coq_Z -> word -> (error, array) result **)
  ...
  (** val get :
      positive -> aligned -> arr_access -> wsize -> array -> coq_Z -> word
      exec **)
  ...
  let fill_aux len =
    foldM (fun w pt ->
      ...
      | Error s -> Error s) (Z0, (empty len))

  (** val fill : positive -> word list -> array exec **)

  let fill len bytes =
    if eq_op coq_Datatypes_nat__canonical__eqtype_Equality
         (Obj.magic size bytes) (Obj.magic Pos.to_nat len)
    then (match fill_aux len bytes with
          | Ok x -> Ok (snd x)
          | Error s -> Error s)
  ...
  let is_uincl x y a b =
    (&&)
      (eq_op coq_BinNums_positive__canonical__eqtype_Equality (Obj.magic x)
        (Obj.magic y))
      (all (fun i ->
        match CoreMem.read coq_BinNums_Z__canonical__eqtype_Equality
                coq_PointerZ (array_CM x) a Aligned (Obj.magic i) U8 with
        | Ok p ->
          (match CoreMem.read coq_BinNums_Z__canonical__eqtype_Equality
                   coq_PointerZ (array_CM y) b Aligned (Obj.magic i) U8 with
           | Ok q ->
             eq_op (word_word__canonical__eqtype_Equality U8) (Obj.magic p)
               (Obj.magic q)
           | Error _ -> false)
        | Error _ -> true) (ziota Z0 (Zpos x)))

  (** val is_uinclP : positive -> positive -> array -> array -> reflect **)

  let is_uinclP x y a b =
    let _evar_0_ = fun _ ->
      let _evar_0_ = fun _ -> ReflectT in
      let _evar_0_0 = fun _ -> ReflectF in
      (match allP coq_BinNums_Z__canonical__eqtype_Equality (fun i ->
               match CoreMem.read coq_BinNums_Z__canonical__eqtype_Equality
                       coq_PointerZ (array_CM y) a Aligned i U8 with
               | Ok p ->
                 (match CoreMem.read
                          coq_BinNums_Z__canonical__eqtype_Equality
                          coq_PointerZ (array_CM y) b Aligned i U8 with
                  | Ok q ->
                    eq_op (word_word__canonical__eqtype_Equality U8)
                      (Obj.magic p) (Obj.magic q)
                  | Error _ -> false)
               | Error _ -> true) (Obj.magic ziota Z0 (Zpos y)) with
       | ReflectT -> _evar_0_ __
       | ReflectF -> _evar_0_0 __)
    in
    let _evar_0_0 = fun _ -> ReflectF in
    (match eqP coq_BinNums_positive__canonical__eqtype_Equality (Obj.magic x)
             (Obj.magic y) with
     | ReflectT -> _evar_0_ __
     | ReflectF -> _evar_0_0 __)
 end
