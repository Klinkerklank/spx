...
(** val tmp_var :
    (v_kind -> instr_info -> string -> atype -> Ident.Ident.ident) ->
    fun_info -> wsize -> Var.var **)

let tmp_var fresh_var_ident =
  let fresh_temporary = fun fi ws ->
    fresh_var_ident (Reg (Normal, Direct)) (entry_info_of_fun_info fi) "tmp"
      (Coq_aword ws)
  in
  (fun fi ws -> { Var.vtype = (Coq_aword ws); Var.vname =
  (fresh_temporary fi ws) })

(** val indirect_copy :
    'a1 asmOp -> (v_kind -> instr_info -> string -> atype ->
    Ident.Ident.ident) -> fun_info -> wsize -> var_i -> gvar -> pexpr -> 'a1
    instr_r list **)

let indirect_copy _ fresh_var_ident fi ws x y i =
  let tmp = { v_var = (tmp_var fresh_var_ident fi ws); v_info = x.v_info } in
...
(** val array_copy :
    'a1 asmOp -> (v_kind -> instr_info -> string -> atype ->
    Ident.Ident.ident) -> fun_info -> instr_info -> var_i -> wsize ->
    positive -> gvar -> 'a1 instr list **)

let array_copy asmop fresh_var_ident =
  let fresh_counter = fun fi ->
    fresh_var_ident Inline (entry_info_of_fun_info fi) "i__copy" Coq_aint
  in
  (fun fi ii x ws n y ->
  let i_name = fresh_counter fi in
  let i = { v_var = { Var.vtype = Coq_aint; Var.vname = i_name }; v_info =
    x.v_info }
  in
  ...
  (map (fun i0 -> MkI (ii, i0))
    (if needs_temporary x.v_var y.gv.v_var
     then indirect_copy asmop fresh_var_ident fi ws x y ei
...
(** val get_source :
    'a1 asmOp -> (v_kind -> instr_info -> string -> atype ->
    Ident.Ident.ident) -> SvExtra.Sv.t -> instr_info -> pexpr list ->
    (gvar * 'a1 instr list) cexec **)
   ...
   | [] ->
     (match e with
      ...
      | Psub (_, ws, len, x, _) ->
        ...
        let y_name =
          fresh_var_ident (Ident.Ident.id_kind (Var.vname x.gv.v_var)) ii
            "src" ty
        in
...
(** val get_target :
    'a1 asmOp -> (v_kind -> instr_info -> string -> atype ->
    Ident.Ident.ident) -> SvExtra.Sv.t -> instr_info -> lval list ->
    (var_i * 'a1 instr list) cexec **)
   ...
   | [] ->
     (match d with
      ...
      | Lasub (_, ws, len, x, _) ->
        let ty = Coq_aarr (ws, len) in
        let x_name =
          fresh_var_ident (Ident.Ident.id_kind (Var.vname x.v_var)) ii "dst"
            ty
        in
...
(** val array_copy_i :
    'a1 asmOp -> (v_kind -> instr_info -> string -> atype ->
    Ident.Ident.ident) -> fun_info -> SvExtra.Sv.t -> 'a1 instr -> 'a1 instr
    list cexec **)

let rec array_copy_i asmop fresh_var_ident fi v i = match i with
| MkI (ii, id) ->
  (match id with
   | Copn (xs, _, o, es) ->
     (match is_copy asmop o with
           ...
           (match get_target asmop fresh_var_ident v ii xs with
            | Ok x0 ->
              ...
              then Ok
                     (cat pre
                       (cat
                         (array_copy asmop fresh_var_ident fi ii x1 ws n y)
                         post))
              else let s =
      ...
      | None -> Ok (i :: []))
   | Cif (e, c1, c2) ->
     (match array_copy_c asmop v (array_copy_i asmop fresh_var_ident fi) c1 with
      | Ok x ->
        (match array_copy_c asmop v (array_copy_i asmop fresh_var_ident fi) c2 with
         | Ok x0 -> Ok ((MkI (ii, (Cif (e, x, x0)))) :: [])
         | Error s -> Error s)
      | Error s -> Error s)
   | Cfor (i0, r, c) ->
     (match array_copy_c asmop v (array_copy_i asmop fresh_var_ident fi) c with
...
(** val array_copy_fd :
    'a1 asmOp -> (v_kind -> instr_info -> string -> atype ->
    Ident.Ident.ident) -> progT -> 'a1 fundef -> (pp_error_loc, ('a1,
    extra_fun_t) _fundef) result **)

let array_copy_fd asmop fresh_var_ident =
  let fresh_counter = fun fi ->
    fresh_var_ident Inline (entry_info_of_fun_info fi) "i__copy" Coq_aint
  in
  (fun pT f ->
  ...
  in
  let v = vars_fd asmop pT f in
  let fresh =
    SvExtra.Sv.add
      (Obj.magic { Var.vtype = Coq_aint; Var.vname = (fresh_counter fi) })
      (SvExtra.sv_of_list (Obj.magic tmp_var fresh_var_ident fi) wsizes)
  in
  if SvExtra.disjoint fresh v
  then (match array_copy_c asmop v (array_copy_i asmop fresh_var_ident fi) c with
        | Ok x ->
...
(** val array_copy_prog :
    'a1 asmOp -> (v_kind -> instr_info -> string -> atype ->
    Ident.Ident.ident) -> progT -> 'a1 prog -> (pp_error_loc, ('a1,
    extra_fun_t, extra_prog_t) _prog) result **)

let array_copy_prog asmop fresh_var_ident pT p =
  match map_cfprog_gen (fun x -> x.f_info)
          (array_copy_fd asmop fresh_var_ident pT) p.p_funcs with
