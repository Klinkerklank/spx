...
(** val init_map :
    ...
    Ident.Ident.ident) -> fun_info -> SvExtra.Sv.t -> Var.var
    Mvar.t * Uint63.t **)

let init_map fresh_var_ident fi s =
  SvExtra.Sv.fold (fun x pat ->
    ...
    let n0 = (^) (Ident.Ident.id_name n) "_spill" in
    ((Mvar.set m x { Var.vtype = ty; Var.vname =
       (fresh_var_ident k (entry_info_of_fun_info fi) count n0 ty) }),
    (succ count))) s (Mvar.empty, (Uint63.of_int (0)))
...
(** val spill_fd :
    ...
    Ident.Ident.ident) -> progT -> funname -> 'a1 fundef -> 'a1 fundef cexec **)

let spill_fd asmop fresh_var_ident pT _ fd =
  let { f_info = fi; f_tyin = tyi; f_params = params; f_body = c; f_tyout =
    tyo; f_res = res; f_extra = ef } = fd
  in
  let s = foldl (to_spill_i asmop) (SvExtra.Sv.empty, false) c in
  if negb (snd s)
  then Ok fd
  else let (m, _) = init_map fresh_var_ident fi (fst s) in
       let x = vars_fd asmop pT fd in
       let b = check_map m x in
       if fst b
       then (match spill_c asmop (spill_i asmop (get_spill m))
                     SvExtra.Sv.empty c with
             | Ok x0 ->
               Ok { f_info = fi; f_tyin = tyi; f_params = params; f_body =
...
let spill_prog asmop fresh_var_ident pT p =
  match map_cfprog_name_gen (fun x -> x.f_info)
          (spill_fd asmop fresh_var_ident pT) p.p_funcs with
