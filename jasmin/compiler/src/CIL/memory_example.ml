...
module MemoryI =
 struct
  ...
  type mem_ = { data : word Mz.t; alloc : unit Mz.t; stk_root : word;
                stk_limit : word; frames : frame list }

  (** val data : coq_PointerData -> mem_ -> word Mz.t **)
  ...
  (** val stk_root : coq_PointerData -> mem_ -> word **)

  let stk_root _ m =
    m.stk_root

  (** val stk_limit : coq_PointerData -> mem_ -> word **)
  ...
  (** val is_alloc : coq_PointerData -> mem -> word -> bool **)

  let is_alloc pd m p =
    is_zalloc m.alloc (wunsigned pd p)

  (** val is_init : coq_PointerData -> mem -> word -> bool **)

  let is_init pd m p =
    is_zalloc m.data (wunsigned pd p)

  (** val get : coq_PointerData -> mem -> word -> (error, word) result **)

  let get pd m p =
    if (&&) (is_alloc pd m p) (is_init pd m p)
    then Ok
           (Ssrfun.Option.default (word0 U8)
             (Mz.get m.data (Obj.magic wunsigned pd p)))
    else let s = ErrAddrInvalid in Error s

  (** val set :
      coq_PointerData -> mem -> word -> word -> (error, mem_) result **)
  ...
  (** val is_allocP : coq_PointerData -> mem -> word -> word -> reflect **)
  ...
  let coq_CM pd =
    { Memory_model.get = (Obj.magic get pd); Memory_model.set =
      (Obj.magic set pd); valid8 = (Obj.magic is_alloc pd); valid8P =
      (Obj.magic is_allocP pd) }

  (** val top_stack : coq_PointerData -> mem -> Equality.sort **)

  let top_stack pd m =
    (coq_PointerW pd).add (Obj.magic m.stk_root)
      (Z.opp (footprint_of_stack m.frames))
  ...
  (** val clear_data : word Mz.t -> coq_Z -> coq_Z -> word Mz.t **)
  ...
  (** val stack_blocks :
      coq_PointerData -> Equality.sort -> frame list -> word list **)

  let stack_blocks pd stk_root0 frames0 =
    snd (Obj.magic stack_blocks_rec pd stk_root0 frames0)

  (** val stack_frames : coq_PointerData -> mem -> word list **)

  let stack_frames pd m =
    stack_blocks pd (Obj.magic m.stk_root) m.frames
  ...
  (** val init_mem_alloc :
      coq_PointerData -> (word * coq_Z) list -> unit Mz.t **)
  ...
  (** val all_above :
      coq_PointerData -> (word * coq_Z) list -> word -> bool **)

  let all_above pd s stk =
    all (fun pat -> let (p, _) = pat in wlt pd Unsigned stk p) s

  (** val init_mem :
      coq_PointerData -> (word * coq_Z) list -> word -> mem exec **)

  let init_mem pd s stk =
    if sumbool_of_bool
         (is_align (word_word__canonical__eqtype_Equality pd)
           (coq_PointerW pd) (Obj.magic stk) U256)
    then if sumbool_of_bool (all_above pd s stk)
         then Ok { data = Mz.empty; alloc = (init_mem_alloc pd s); stk_root =
                stk; stk_limit = (word0 pd); frames = [] }
         else Error ErrStack
    else Error ErrStack
