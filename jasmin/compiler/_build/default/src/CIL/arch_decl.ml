...
(** val check_CAimm :
    ('a1, 'a2, 'a3, 'a4, 'a5) arch_decl -> caimm_checker_s -> wsize -> word
    -> bool **)
...
type ('reg, 'regx, 'xreg, 'rflag, 'cond) reg_address = { ad_disp : word;
                                                         ad_base : ('reg,
                                                                   'regx,
                                                                   'xreg,
                                                                   'rflag,
...
let reg_address_beq arch addr1 addr2 =
  ...
  (&&)
    (eq_op (word_word__canonical__eqtype_Equality (arch_pd arch))
...
let address_eq_axiom arch _top_assumption_ =
  let _evar_0_ = fun _r_ __top_assumption_ ->
    ...
    (match __top_assumption_ with
     ...
     | Arip w -> _evar_0_0 w)
  in
  let _evar_0_0 = fun _w_ __top_assumption_ ->
    let _evar_0_0 = fun _ -> ReflectF in
    let _evar_0_1 = fun _w1_ ->
      reflect_inj (word_word__canonical__eqtype_Equality arch.reg_size)
        (Obj.magic (fun x -> Arip x)) _w_ _w1_
        (eqP (word_word__canonical__eqtype_Equality arch.reg_size) _w_ _w1_)
    in
    (match __top_assumption_ with
     | Areg r -> _evar_0_0 r
     | Arip w -> Obj.magic _evar_0_1 w)
  in
  (match _top_assumption_ with
   | Areg r -> Obj.magic _evar_0_ r
   | Arip w -> Obj.magic _evar_0_0 w)
...
let wextend_size ws t x =
  ...
  | Coq_lbool -> x
  | Coq_lword ws' ->
    if cmp_le wsize_cmp ws' ws then Obj.magic zero_extend ws ws' x else x
...
(** val asm_globs :
    ('a1, 'a2, 'a3, 'a4, 'a5) arch_decl -> ('a1, 'a2, 'a3, 'a4, 'a5, 'a6)
    asm_op_decl -> ('a1, 'a2, 'a3, 'a4, 'a5, 'a6) asm_prog -> word list **)
...
(** val eval_cond :
    ...
    word) -> (('a1, 'a2, 'a3, 'a4, 'a5) rflag_t -> bool exec) -> ('a1, 'a2,
    'a3, 'a4, 'a5) cond_t -> bool exec **)
