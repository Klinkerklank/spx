...
let check_shift_amount sz e =
  ...
  | Some n ->
    if eq_op (word_word__canonical__eqtype_Equality U8) (Obj.magic n)
         (Obj.magic wand U8 n (x86_shift_mask sz))
    then Some e
    else None
  | None ->
    (match e with
          ...
          (match is_wconst U8 b with
           | Some n ->
             if eq_op (word_word__canonical__eqtype_Equality U8)
                  (Obj.magic n) (Obj.magic x86_shift_mask sz)
             then Some a
             else None
           | None -> None)
        | _ -> None)
...
(** val check_wide_shift_amount : velem -> pexpr -> pexpr option **)

let check_wide_shift_amount ve e = match e with
| Papp1 (s, p) ->
  (match s with
   | Oword_of_int w ->
     (match w with
      | U128 ->
        (match p with
         | Pconst z ->
           if (&&) (Z.leb Z0 z) (Z.leb z (wsize_bits (wsize_of_velem ve)))
           then Some e
           else None
         | _ -> None)
      | _ -> None)
                 ...
                 (match p0 with
                  | Pconst z ->
                    if eq_op coq_BinNums_Z__canonical__eqtype_Equality
                         (Obj.magic z) (Obj.magic wmax_unsigned U64)
                    then Some a
                    else None
                  | _ -> None)
               | _ -> None)
  ...
  (match e with
        ...
        (match szi with
         ...
         | U32 ->
           (match szo with
            ...
            | U128 ->
              kb true szo (LowerCopn ((coq_Ox86 atoI (VMOV szi)), (a :: [])))
            | U256 ->
         ...
         | U128 ->
           (match szo with
            | U256 ->
              kb (negb (is_lval_in_memory x)) szo (LowerCopn ((Oasm (BaseOp
                ((Some szo), (VMOVDQU szi)))), (a :: [])))
            | _ -> LowerAssgn)
         | U256 -> LowerAssgn)
   ...
   | Papp2 (op, a, b) ->
     (match op with
      ...
      | Ovlsr (ve, sz) ->
        (match check_wide_shift_amount ve b with
         | Some b0 ->
           kb
             ((&&) (cmp_le wsize_cmp U16 (wsize_of_velem ve))
               (cmp_le wsize_cmp U128 sz)) sz (LowerCopn
             ((coq_Ox86 atoI (VPSRL (ve, sz))), (a :: (b0 :: []))))
         | None -> LowerAssgn)
      | Ovlsl (ve, sz) ->
        (match check_wide_shift_amount ve b with
         | Some b0 ->
           kb
             ((&&) (cmp_le wsize_cmp U16 (wsize_of_velem ve))
               (cmp_le wsize_cmp U128 sz)) sz (LowerCopn
             ((coq_Ox86 atoI (VPSLL (ve, sz))), (a :: (b0 :: []))))
         | None -> LowerAssgn)
      | Ovasr (ve, sz) ->
        (match check_wide_shift_amount ve b with
         | Some b0 ->
           kb
             ((&&) (size_16_32 (wsize_of_velem ve))
               (cmp_le wsize_cmp U128 sz)) sz (LowerCopn
             ((coq_Ox86 atoI (VPSRA (ve, sz))), (a :: (b0 :: []))))
         | None -> LowerAssgn)
...
let lower_cassgn atoI options warning fv =
  ...
  (match lower_cassgn_classify atoI ty e x with
   ...
   | LowerLea (sz, l) ->
     ...
     let b0 = Ssrfun.Option.apply coq_Plvar (wconst sz (word0 sz)) b in
     let o0 = Ssrfun.Option.apply coq_Plvar (wconst sz (word0 sz)) o in
     let lea0 = fun _ ->
     ...
     then lea0 ()
          ...
          then if eq_op coq_BinNums_Z__canonical__eqtype_Equality
                    (Obj.magic sc) (Obj.magic (Zpos Coq_xH))
               then (MkI (ii, (Copn
                      ((f :: (f :: (f :: (f :: (f :: (x :: [])))))), tg,
...
| MkI (ii, ir) ->
  (match ir with
   ...
   | Cassert _ -> map (fun x -> MkI (ii, x)) (ir :: [])
   | Cif (e, c1, c2) ->
