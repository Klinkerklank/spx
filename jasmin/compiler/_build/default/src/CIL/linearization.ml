  ...
  (match ir with
   ...
   | Copn (xs, _, _, es) ->
     (match allM (check_rexpr ii) es with
      ...
      | Error s -> Error s)
   | Csyscall (_, _, _) -> Ok ()
   | Cassert _ -> Error (E.ii_error ii "assert found in linear")
   | Cif (b, c1, c2) ->
  ...
  match foldM (fun a base ->
          ...
          | Ok x ->
            ...
            then if cmp_le wsize_cmp ws al
                 then if is_align (word_word__canonical__eqtype_Equality pd)
                           (coq_PointerW pd) (Obj.magic wrepr pd ofs) ws
