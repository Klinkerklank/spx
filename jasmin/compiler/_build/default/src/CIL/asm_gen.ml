      ...
      | Oword_of_int sz' ->
        (match f0 with
         | Fconst z ->
           ...
           if eq_op (word_word__canonical__eqtype_Equality sz) (Obj.magic w1)
                (Obj.magic w2)
           then Ok (Imm (sz', w))
          ...
          (match Obj.magic a' with
           | Imm (sz2, w2) ->
             eq_op (word_word__canonical__eqtype_Equality sz)
               (Obj.magic sign_extend sz sz1 w1)
               (Obj.magic sign_extend sz sz2 w2)
           | _ -> false)
        | _ -> false))
  ...
  | Imm (sz, w) ->
    (match cond with
     | CAimm (checker, sz') ->
       ...
       if (&&)
            (eq_op (word_word__canonical__eqtype_Equality sz) (Obj.magic w)
              (Obj.magic w2))
