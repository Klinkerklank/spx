...
let fun_of_fin_rec aT x s f_s =
  ...
  | Coq_finfun_cons (x1, s1, y1, f_s1) ->
    (match eqP (Finite.Exports.fintype_Finite__to__eqtype_Equality aT) x0 x1 with
     | ReflectT -> (fun _ -> y1)
...
module type Coq_finfun_Locked =
 sig
  val body : Finite.coq_type -> (Finite.sort -> 'a1) -> 'a1 finfun_of
 end
