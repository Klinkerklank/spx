...
let rec sexpr_beq e1 e2 =
  ...
  | Sneg (opk1, e3) ->
    (match e2 with
     | Sneg (opk2, e4) ->
       (&&)
         (eq_op operators_op_kind__canonical__eqtype_Equality
           (Obj.magic opk1) (Obj.magic opk2)) (sexpr_beq e3 e4)
     | _ -> false)
  | Sadd (opk1, e11, e12) ->
    (match e2 with
     | Sadd (opk2, e21, e22) ->
...
let sexpr_eq_axiom _top_assumption_ =
  ...
  let _evar_0_3 = fun opk1 e1 _ __top_assumption_ ->
    ...
    let _evar_0_7 = fun opk2 e2 ->
      iffP
        ((&&) (eq_op operators_op_kind__canonical__eqtype_Equality opk1 opk2)
          (sexpr_beq e1 e2))
        (andP (eq_op operators_op_kind__canonical__eqtype_Equality opk1 opk2)
          (sexpr_beq e1 e2))
    in
    let _evar_0_8 = fun _ _ _ -> ReflectF in
    let _evar_0_9 = fun _ _ _ -> ReflectF in
  ...
  (match ir with
   ...
   | Csyscall (rs, o, es) ->
     ...
     (match alloc_syscall pd asmop saparams pmap ii rmap rs o es with
      ...
      | Error s -> Error s)
   | Cassert _ ->
     Error (pp_at_ii ii (E.stk_ierror_no_var "don't deal with assert"))
   | Cif (e, c1, c2) ->
...
(** val check_glob :
    word list -> glob_value -> (pp_error_loc, unit) result **)

let check_glob data = function
| Gword (ws, w) ->
  if eq_op (word_word__canonical__eqtype_Equality ws)
       (Obj.magic LE.decode ws data) (Obj.magic w)
  then Ok ()
...
(** val init_map :
    ((Var.var * wsize) * coq_Z) list -> word list -> glob_decl list ->
    (coq_Z * wsize) Mvar.t cexec **)
...
(** val alloc_prog :
    ...
    'a1 stack_alloc_params -> ('a1 asm_op_t -> bool) -> (v_kind -> Uint63.t
    -> string -> atype -> Ident.Ident.ident) -> (sub_region -> pp_error) ->
    Ident.Ident.ident -> Ident.Ident.ident -> word list ->
