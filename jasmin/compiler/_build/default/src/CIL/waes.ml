...
let coq_SubWord v1 =
  make_vec U8 U32 (map coq_Sbox (split_vec U32 (nat_of_wsize U8) v1))

(** val coq_InvSubWord : Word0.word -> Word0.word **)

let coq_InvSubWord v1 =
  make_vec U8 U32 (map coq_InvSbox (split_vec U32 (nat_of_wsize U8) v1))

(** val coq_RotWord : Word0.word -> Word0.word **)

let coq_RotWord v1 =
  make_vec U8 U32
    ((subword (S (wsize_size_minus_1 U32)) (muln (S O) (nat_of_wsize U8))
       (nat_of_wsize U8) v1) :: ((subword (S (wsize_size_minus_1 U32))
                                   (muln (S (S O)) (nat_of_wsize U8))
                                   (nat_of_wsize U8) v1) :: ((subword (S
                                                               (wsize_size_minus_1
                                                                 U32))
                                                               (muln (S (S (S
                                                                 O)))
                                                               ...
                                                               (nat_of_wsize
                                                                 U8) v1) :: (
    (subword (S (wsize_size_minus_1 U32)) (muln O (nat_of_wsize U8))
      (nat_of_wsize U8) v1) :: []))))
...
let coq_SubBytes s =
  make_vec U32 U128 (map coq_SubWord (split_vec U128 (nat_of_wsize U32) s))

(** val coq_InvSubBytes : Word0.word -> Word0.word **)

let coq_InvSubBytes s =
  make_vec U32 U128 (map coq_InvSubWord (split_vec U128 (nat_of_wsize U32) s))
...
let coq_ShiftRows s =
  ...
  let (p9, s11) = p8 in
  let (p10, s10) = p9 in
  to_state ((((((((((((p10, s11), s12), s13), s10), s22), s23), s20), s21),
    s33), s30), s31), s32)
...
let coq_InvShiftRows s =
  ...
  let (p9, s12) = p8 in
  let (p10, s11) = p9 in
  to_state ((((((((((((p10, s10), s11), s12), s13), s20), s21), s22), s23),
    s30), s31), s32), s33)
...
let wAESKEYGENASSIST v1 v2 =
  ...
  let x3 =
    subword (S (wsize_size_minus_1 U128))
      (muln (S (S (S O))) (nat_of_wsize U32)) (nat_of_wsize U32) v1
  in
  let y0 = coq_SubWord x1 in
  let y1 = Word0.wxor U32 (coq_RotWord (coq_SubWord x1)) rcon in
  let y2 = coq_SubWord x3 in
  let y3 = Word0.wxor U32 (coq_RotWord (coq_SubWord x3)) rcon in
