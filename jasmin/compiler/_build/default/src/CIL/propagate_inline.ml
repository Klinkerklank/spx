...
let remove pi x =
  Mvar.filter_map (fun y c ->
    if (||)
         (eq_op Var.coq_MvMake_var__canonical__eqtype_Equality (Obj.magic x)
           y) (SvExtra.Sv.mem (Obj.magic x) c.pi_fv)
    then None
    else Some c) pi
...
| Pget (al, aa, ws, x, e0) -> Pget (al, aa, ws, x, (pi_e fcp pi e0))
| Psub (aa, ws, len, x, e0) -> Psub (aa, ws, len, x, (pi_e fcp pi e0))
| Pload (al, ws, e0) -> Pload (al, ws, (pi_e fcp pi e0))
...
| Lmem (al, ws, vi, e) ->
  ((remove_m pi), (Lmem (al, ws, vi, (pi_e fcp pi e))))
| Laset (al, aa, ws, x, e) ->
  ((remove pi x.v_var), (Laset (al, aa, ws, x, (pi_e fcp pi e))))
| Lasub (aa, ws, len, x, e) ->
...
let set_lv pi x tag e =
  match x with
  | Lvar x0 ->
    if eq_op expr_assgn_tag__canonical__eqtype_Equality tag
         (Obj.magic AT_inline)
...
| MkI (ii, ir) ->
  (match ir with
   ...
   | Cassert a ->
     let (msg, e) = a in
     let e0 = pi_e fcp pi e in Ok (pi, (MkI (ii, (Cassert (msg, e0)))))
   | Cif (e, c1, c2) ->
     let e0 = pi_e fcp pi e in
   ...
   | Cwhile (a, c1, e, info, c2) ->
     (match loop_while asmop fcp (pi_i asmop fcp) ii c1 e c2 Loop.nb pi with
      | Ok x ->
        ...
        let (y0, e0) = y in
        let (pi0, c4) = y0 in
        Ok (pi0, (MkI (ii, (Cwhile (a, c4, e0, info, c3)))))
      | Error s -> Error s)
   | Ccall (xs, f, es) ->
     let es0 = pi_es fcp pi es in
