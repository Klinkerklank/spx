...
(** val se_init_sem : coq_MSFsize -> word **)

let se_init_sem =
  word0

(** val se_update_sem : coq_MSFsize -> bool -> word -> word **)

let se_update_sem msfsz b msf =
  if b then msf else opp_word msfsz (word1 msfsz)

(** val se_move_sem : coq_MSFsize -> word -> word **)

let se_move_sem _ w =
  w

(** val se_protect_sem : coq_MSFsize -> wsize -> word -> word -> word **)

let se_protect_sem _ _ w _ =
  w

(** val se_protect_ptr_sem :
    coq_MSFsize -> positive -> WArray.array -> word -> WArray.array **)

let se_protect_ptr_sem _ _ t _ =
  t

(** val se_protect_ptr_fail_sem :
    coq_MSFsize -> positive -> WArray.array -> word -> WArray.array exec **)

let se_protect_ptr_fail_sem msfsz _ t msf =
  if eq_op (word_word__canonical__eqtype_Equality msfsz) (Obj.magic msf)
       (Obj.magic word0 msfsz)
  then Ok t
...
let coq_SLHinit_instr msfsz =
  { str = (pp_s coq_SLHinit_str); tin = []; i_in = []; tout = ((Coq_aword
    ...
    semi = (sem_prod_ok (map eval_atype []) (Obj.magic se_init_sem msfsz));
    i_valid = true; i_safe = [] }
